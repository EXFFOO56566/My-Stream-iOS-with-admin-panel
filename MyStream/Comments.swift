    ...
    if commentTxt.text == "" {
        ...
        cObj.saveInBackground(block: { (succ, error) in
            if error == nil {
                ...
                self.sendPushNotification(currentUser: currUser, pushMess: "commented on your Stream: ", textColumn: STREAMS_TEXT, obj: self.sObj, userPointerColumn: STREAMS_USER_POINTER)
                
                // Save Activity
                self.saveActivity(currUser: currUser, streamObj: self.sObj, text: "commented on your Stream: '\(self.sObj[STREAMS_TEXT]!)'")
    ...
    userPointer.fetchIfNeededInBackground(block: { (user, error) in
        if error == nil {
            ...
            let reportComment = UIAlertAction(title: "Report Comment", style: .default, handler: { (action) -> Void in
                ...
                let alert = UIAlertController(title: APP_NAME,
                    message: "Are you sure you want to report this comment to the Admin?",
                    preferredStyle: .alert)
