...
@IBAction func shareStreamButt(_ sender: Any) {
    let messageStr  = "\(sObj[STREAMS_TEXT]!) on #\(APP_NAME)"
    var img = UIImage()
...
@IBAction func optionsButt(_ sender: Any) {
    ...
    userPointer.fetchIfNeededInBackground(block: { (user, error) in
        if error == nil {
            ...
            // REPORT STREAM -----------------------------------
            let reportStream = UIAlertAction(title: "Report Stream", style: .default, handler: { (action) -> Void in
                
                    let alert = UIAlertController(title: APP_NAME,
                        message: "Are you sure you want to report this Stream to the Admin?",
                        preferredStyle: .alert)
                
                    let ok = UIAlertAction(title: "Report", style: .default, handler: { (action) -> Void in
                    
                        self.showHUD("Reporting Stream...")
                        ...
                        self.sObj.saveInBackground(block: { (succ, error) in
                            if error == nil {
                                self.hideHUD()
                                self.simpleAlert("Thanks for reporting this Stream, we'll check it out withint 24 hours!")
                                mustRefresh = true
                        }})
                    })
            ...
            if userPointer.objectId == currUser.objectId {
                let deleteStream = UIAlertAction(title: "Delete Stream", style: .destructive, handler: { (action) -> Void in
