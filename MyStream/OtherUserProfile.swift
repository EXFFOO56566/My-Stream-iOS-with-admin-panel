...
override func viewDidLoad() {
    ...
    followersLabel.layer.cornerRadius = followersLabel.bounds.size.width/2
    followersLabel.layer.borderColor = UIColor.white.cgColor
    followersLabel.layer.borderWidth = 3
    
    followingLabel.layer.cornerRadius = followingLabel.bounds.size.width/2
    followingLabel.layer.borderColor = UIColor.white.cgColor
    followingLabel.layer.borderWidth = 3
    
    followOutlet.layer.cornerRadius = 14
    ...
    if followOutlet.titleLabel?.text == "Follow" {
        ...
        fObj.saveInBackground(block: { (succ, error) in
            if error == nil {
                ...
                let pushStr = "\(currUser[USER_USERNAME]!) started to follow you!"
...
@IBAction func reportUserButt(_ sender: Any) {
    ...
    let alert = UIAlertController(title: APP_NAME,
        message: "Are you sure you want to report @\(userObj[USER_USERNAME]!) to the Admin?",
